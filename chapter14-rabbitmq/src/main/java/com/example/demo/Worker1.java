    ...
    public static void main(String[] argv) throws Exception {
        ...
        final Consumer consumer2 = new DefaultConsumer(channel) {
            ...
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                ...
                System.out.println(new SimpleDateFormat("HH:mm:ss").format(new Date()) + "Worker2 [x] Received '" + message + "'");
                try {
                    doWork(message);
                } finally {
                    System.out.println(new SimpleDateFormat("HH:mm:ss").format(new Date()) + "Worker2 [x] Done");
                    // 消息处理完成确认
                    channel.basicAck(envelope.getDeliveryTag(), false);
                }
            }
